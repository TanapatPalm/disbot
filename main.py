...
# 🔄 TASKS (Update Dashboard & Notify)

@tasks.loop(seconds=10)
async def update_dashboard():
    # ฟังก์ชันนี้สำหรับอัปเดต Dashboard (แต่ตอนนี้เราใช้ myserver แบบง่าย Dashboard นี้อาจจะไม่แสดงผลในเว็บ แต่โค้ดบอทจะไม่พัง)
    pass
...
async def check_schedule():
    ...
    # ใช้ try-except กันบอทดับถ้า DB หลุด
    try:
...
# --- CREATE JOB (รวมโค้ดให้แล้ว ห้ามแยก Decorator) ---
...
async def create_job(
    ...
    global pool
    async with pool.acquire() as conn:
        # ใช้ Syntax ของ PostgreSQL (RETURNING job_id)
